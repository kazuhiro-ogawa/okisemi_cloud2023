...
static int getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  (void)az_span_dtoa(payload, temp, 1, &payload);
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"humidity\":"));
  (void)az_span_dtoa(payload, humi, 1, &payload);

#if 0
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"alert\":"));
  if(temp > 32 && humi > 80){
    (void)az_span_dtoa(payload, 1, 1, &payload);
  }
  else{
    (void)az_span_dtoa(payload, 0, 1, &payload);
  }
#endif
...
static void sendTelemetry()
{
  ...
  int alert = getTelemetryPayload(telemetry, &telemetry);

  // Allocate a span to put the properties.
  uint8_t property_buffer[32];
  ...
  // Determine the string representation based on the value of alert.
  const char* alert_str = (alert == 0) ? "false" : "true";

  // Create an az_span manually from the alert string.
  az_span alert_span = az_span_create((uint8_t*)alert_str, strlen(alert_str));

  // Append properties.
  az_iot_message_properties_append(
      &props, AZ_SPAN_LITERAL_FROM_STR("alert"), alert_span);
  ...
  Logger.Info((const char*)az_span_ptr(telemetry));
  Logger.Info(telemetry_topic);
  Logger.Info((const char*)telemetry_payload);
  Logger.Info((const char*)sas_signature_buffer);
